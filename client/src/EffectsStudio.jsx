...
export default function EffectsStudio() {
  const { ready, sendFrame } = useKeyboard();
    ...
    <>
      <section className="panel">
        ...
        <div className="sliders">
          ...
          <label>Speed <input type="range" min="0" max="200" value={speed} onChange={(e) => setSpeed(+e.target.value)} /></label>
          <label>Brightness <input type="range" min="0" max="100" value={brightness} onChange={(e) => setBrightness(+e.target.value)} /></label>
          <label>Max FPS <input type="range" min="5" max="60" value={maxFps} onChange={(e) => setMaxFps(+e.target.value)} /><b>{maxFps}</b></label>
        </div>
      </section>
      ...
      <section className="board-wrap">
        <div className="board" style={{ gridTemplateColumns: `repeat(${GRID.cols}, 1fr)` }}>
          {KEYS.map((k) => (
            <div key={k.index} className="key" style={{ gridColumn: k.col + 1, gridRow: k.row + 1 }}
